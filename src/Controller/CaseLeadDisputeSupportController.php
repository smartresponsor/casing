...
use App\Casing\Service\CaseLeadDisputeIntakeService;
...
final readonly class CaseLeadDisputeSupportController
{
    public function __construct(
        ...
        private CaseLeadDisputeIntakeService $disputes,
    ...
    public function intake(Request $request): array
    {
        ...
        $claim = new CaseLeadDisputeClaimDTO();
        $form = $this->forms->create(CaseLeadDisputeClaimType::class, $claim, ['subjects' => $subjects, 'types' => $types]);

        if ($request->isMethod('POST')) {
            $form->submit($this->requestPayload($request));
            if ($form->isValid() && $claim->subject instanceof CaseLeadSubject) {
    ...
    public function contextual(Request $request, string $leadReference): array
    {
        ...
        if (!$subject instanceof CaseLeadSubject) {
        ...
        $form = $this->forms->create(CaseLeadDisputeClaimType::class, $claim, ['subjects' => [$subject], 'types' => $types]);
        if ($request->isMethod('POST')) {
    ...
    /** @param list<CaseLeadSubject> $subjects */
    private function claimFromDraft(CaseDraftEntity $draft, array $subjects): CaseLeadDisputeClaimDTO
    {
        $claim = new CaseLeadDisputeClaimDTO();
     ...
     * @return array<string, mixed>
     */
    private function formPayload(CaseLeadDisputeClaimDTO $claim, array $subjects, array $types, ?string $draftReference): array
    {
        $subjectOptions = array_map(static fn (CaseLeadSubject $subject): array => [
        ...
        return [
            ...
            'data' => [
                ...
                'method' => 'POST',
                'formFields' => [
                    ['nameEntity' => 'subject', 'label' => 'Lead', 'type' => 'select', 'value' => $claim->subject instanceof CaseLeadSubject ? hash('sha256', $claim->subject->leadReference) : null, 'required' => true, 'options' => $subjectOptions],
