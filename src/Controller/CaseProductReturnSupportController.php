...
final readonly class CaseProductReturnSupportController
{
    public function __construct(
        ...
        private CaseProductReturnIntakeService $returns,
    ...
    public function intake(Request $request): array
    {
        ...
        $claim = new CaseProductReturnClaimDTO();
        $form = $this->forms->create(CaseProductReturnClaimType::class, $claim, ['subjects' => $subjects, 'types' => $types]);

        if ($request->isMethod('POST')) {
            $form->submit($this->requestPayload($request));
            if ($form->isValid() && $claim->subject instanceof CasePurchasedProductSubject) {
    ...
    public function contextual(Request $request, string $orderReference, string $itemReference): array
    {
        ...
        if (!$subject instanceof CasePurchasedProductSubject) {
        ...
        $form = $this->forms->create(CaseProductReturnClaimType::class, $claim, ['subjects' => [$subject], 'types' => $types]);
        if ($request->isMethod('POST')) {
    ...
    /** @param list<CasePurchasedProductSubject> $subjects */
    private function claimFromDraft(CaseDraftEntity $draft, array $subjects): CaseProductReturnClaimDTO
    {
        $claim = new CaseProductReturnClaimDTO();
    ...
    {
        $claim = $form->getData();
        $claim = $claim instanceof CaseProductReturnClaimDTO ? $claim : new CaseProductReturnClaimDTO();
        $options = array_map(static fn (CasePurchasedProductSubject $subject): array => [
        ...
        return [
            ...
            'data' => [
                ...
                'method' => 'POST',
                'formFields' => [
                    ['nameEntity' => 'subject', 'label' => 'Purchased product', 'type' => 'select', 'value' => $claim->subject instanceof CasePurchasedProductSubject ? self::subjectToken($claim->subject) : null, 'placeholder' => 'Select a purchased product', 'helpText' => null, 'required' => true, 'validationState' => null, 'errorText' => null, 'options' => $options],
                    ['nameEntity' => 'typeCode', 'label' => 'Return type', 'type' => 'select', 'value' => $claim->typeCode, 'placeholder' => 'Select a return type', 'helpText' => null, 'required' => true, 'validationState' => null, 'errorText' => null, 'options' => $typeOptions],
    ...
    private function requestPayload(Request $request): array
    {
        ...
        return array_intersect_key($all, array_flip(['subject', 'typeCode', 'reason', 'quantity']));
    }
    ...
    private static function subjectToken(CasePurchasedProductSubject $subject): string
    {
