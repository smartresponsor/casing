...
final class CaseContextualSupportControllerTest extends TestCase
{
    public function testOrderContextExposesOnlyActorOwnedHelpActions(): void
    {
        $products = new class implements CasePurchasedProductSubjectResolverInterface {
            ...
            {
                return new CasePurchasedProductSubject('order-1', 'ORD-1', 'SKU-1', 1, 'USD', '25.00', 'completed');
            }
        };
        $payments = new class implements CaseServicePaymentSubjectResolverInterface {
            public function listForActor(string $actorId): array
            {
                return 'actor-1' === $actorId ? [new CaseServicePaymentSubject('payment-1', 'order-1', 'ORD-1', 'completed', '25.00', 'USD', null)] : [];
            }
        ...
        $controller = new CaseContextualSupportController(new CaseActorAccessService($security), $products, $payments);
